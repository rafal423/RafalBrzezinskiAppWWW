...
def context_data():
    context = {
        'site_name': 'Wiadomości',
        'page' : 'home',
        'page_title' : 'Strona wiadomości',
    ...
    if request.POST:
        ...
        if user is not None:
            if user.is_active:
                ...
                resp['msg'] = "Nieprawidłowa nazwa użytkownika lub hasło"
        else:
            resp['msg'] = "Nieprawidłowa nazwa użytkownika lub hasło"
...
def update_profile(request):
    ...
    if not request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "Profil zmieniony")
...
def delete_post(request, pk=None):
    ...
    try:
        ...
        resp['msg'] = 'ID postu jest niewłaściwy'
...
def save_comment(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            resp['id'] = commentID
            resp['status'] = 'success'
            messages.success(request, "Komentarz zapisany.")
        else:
            for field in form:
                for error in field.errors:
                    if not resp['msg'] == '':
...
def delete_comment(request, pk=None):
    resp = {'status': 'failed', 'msg': ''}
    if pk is None:
        resp['msg'] = 'Niewłaściwy ID komentarza'
        return HttpResponse(json.dumps(resp), content_type="application/json")
    try:
